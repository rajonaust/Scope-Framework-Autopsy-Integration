...
def run_SCOPE(input_json):
    ...
    def find_probability(word_list):
        ...
        bow_vector = dictionary.doc2bow(word_list)
        topic_dist = lda_model.get_document_topics(bow_vector, minimum_probability=0)
        pro_list = [0.0] * len(topic)
        for topic_id, prob in topic_dist:
            if topic_id < len(pro_list):
                pro_list[topic_id] = prob
        # Softmax Function
        total_sum = 0
    ...
    chat_messages = []
    for w in process_sentences:
        chat_messages.append(w[4])
    dictionary = corpora.Dictionary(chat_messages)
    corpus = [dictionary.doc2bow(text) for text in chat_messages]
    lda_model = models.LdaModel(corpus=corpus, id2word=dictionary, num_topics=len(topic), passes=10, random_state=42)
    ...
    json_output = pd.DataFrame(all_data).to_json(orient='records', indent=2)
    return json_output
