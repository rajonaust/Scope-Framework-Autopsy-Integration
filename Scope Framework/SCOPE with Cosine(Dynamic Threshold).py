...
def run_SCOPE(input_json):
    ...
    def dynamic_threshold(Threshold_select):
        ...
        standard_probability = Threshold_select
        wall_time = 0
        ...
        if len(overlap_df) != 0:
            ...
            return len(final_df), json_output
        return 0, overlap_df.to_json(orient='records', indent=2)
    
    l = 0.01
    r = 0.10
    cl, dump = dynamic_threshold(l)
    cr, dump = dynamic_threshold(r)
    best = l
    cbest = cl
    while r - l >= 0.001 :
        m1 = l + (r-l)/3
        m2 = r - (r-l)/3
        cm1, dump = dynamic_threshold(m1)
        cm2, dump = dynamic_threshold(m2)
        if cm1 < cl:
            r = m1
            cr = cm1
        else:
