...
class CommAccountMessageModule(DataSourceIngestModule):
	...
	scopeScript = r"\\Mac\Shared Folder Windows\Forensics Tool Plug-In\Source Code\SCOPE_LDA.py"         # FULL path to SCOPE.py
	...
	# REGISTER CUSTOM ARTIFACT TYPE
        ...
        art_name = "TSK_SCOPE_SESSION_LDA"
        art_display = "SCOPE Framework (LDA)"
	...
	def java_clean_text(text):
		...
		try:
        	...
        	attrs.add(BlackboardAttribute(ATTR_USER, "SCOPE Framework (LDA)", str(entry.get("User", "Unknown"))))
		attrs.add(BlackboardAttribute(ATTR_START, "SCOPE Framework (LDA)", str(entry.get("Start Date", ""))))
            	attrs.add(BlackboardAttribute(ATTR_END, "SCOPE Framework (LDA)", str(entry.get("End Date", ""))))
            	attrs.add(BlackboardAttribute(ATTR_DURATION, "SCOPE Framework (LDA)", str(entry.get("Time Duration", "Unknown"))))
            	attrs.add(BlackboardAttribute(ATTR_TOPIC, "SCOPE Framework (LDA)", str(entry.get("Topic", "Unknown"))))
            	attrs.add(BlackboardAttribute(ATTR_PROB, "SCOPE Framework (LDA)", prob))
		attrs.add(BlackboardAttribute(ATTR_SUMMARY, "SCOPE Framework (LDA)",java_clean_text(entry.get("Chat Summary", ""))))
		...
		try:
			blackboard.postArtifact(art, "SCOPE Framework (LDA)")
		except Exception as ex:
			IngestServices.getInstance().postMessage(
        			IngestMessage.createMessage(IngestMessage.MessageType.WARNING, "SCOPE", "Artifact Type was None"+str(ex))
    			)
	...
	try: 
		...
		if scopeArtifactType is not None:
            		IngestServices.getInstance().fireModuleDataEvent(
                		ModuleDataEvent("SCOPE Framework (LDA)", scopeArtifactType)
            		)
        	else:
            		IngestServices.getInstance().postMessage(
