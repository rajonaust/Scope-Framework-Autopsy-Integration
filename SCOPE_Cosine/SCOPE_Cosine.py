...
class CommAccountMessageModule(DataSourceIngestModule):
	...
	scopeScript = r"\\Mac\Shared Folder Windows\Forensics Tool Plug-In\Source Code\SCOPE with Cosine(Main).py"         # FULL path to SCOPE.py
	...
	# REGISTER CUSTOM ARTIFACT TYPE
        ...
        art_name = "TSK_SCOPE_SESSION_Cosine"
        art_display = "SCOPE Framework (Cosine)"
	...
	def java_clean_text(text):
		...
		try:
        	...
        	attrs.add(BlackboardAttribute(ATTR_USER, "SCOPE Framework (Cosine)", str(entry.get("User", "Unknown"))))
		attrs.add(BlackboardAttribute(ATTR_START, "SCOPE Framework (Cosine)", str(entry.get("Start Date", ""))))
            	attrs.add(BlackboardAttribute(ATTR_END, "SCOPE Framework (Cosine)", str(entry.get("End Date", ""))))
            	attrs.add(BlackboardAttribute(ATTR_DURATION, "SCOPE Framework (Cosine)", str(entry.get("Time Duration", "Unknown"))))
            	attrs.add(BlackboardAttribute(ATTR_TOPIC, "SCOPE Framework (Cosine)", str(entry.get("Topic", "Unknown"))))
            	attrs.add(BlackboardAttribute(ATTR_PROB, "SCOPE Framework (Cosine)", prob))
		attrs.add(BlackboardAttribute(ATTR_SUMMARY, "SCOPE Framework (Cosine)",java_clean_text(entry.get("Chat Summary", ""))))
		...
		try:
			blackboard.postArtifact(art, "SCOPE Framework (Cosine)")
		except Exception as ex:
			IngestServices.getInstance().postMessage(
        			IngestMessage.createMessage(IngestMessage.MessageType.WARNING, "SCOPE", "Artifact Type was None"+str(ex))
    			)
	...
	try: 
		...
		if scopeArtifactType is not None:
            		IngestServices.getInstance().fireModuleDataEvent(
                		ModuleDataEvent("SCOPE Framework (Cosine)", scopeArtifactType)
            		)
        	else:
            		IngestServices.getInstance().postMessage(
